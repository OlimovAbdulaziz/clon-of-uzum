        ...
        <div className="products">
            {data?.map((item) => (
                <div key={item.id} className="card">
                    <Link to={`/product/${item.id}`} state={item} className="card__image"><img src={item.url} alt="" /></Link><button className="card__heart">
                        {
                            heart.some(i => i.id === item.id) ?
                                <AiFillHeart style={{color: "#7000FA"}} onClick={() => dispatch(removeFromHeart(item))} /> :
